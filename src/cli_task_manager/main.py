...
def add(
    ...
    priority: Annotated[Priority, typer.Option("--priority", "-p", help="Priority of the task")] = Priority.LOW,
    due: Annotated[str, typer.Option("--due", "-d", help="Due date in format YYYY-MM-DD")] = "2026-12-31"
    ) -> None:
...
@app.command()
def in_progress(task_id: Annotated[int, typer.Argument(help="ID of the task to be in progress")]) -> None:
...
@app.command()
def done(task_id: Annotated[int, typer.Argument(help="Task ID to mark it DONE")]) -> None:
...
@app.command()
def delete(task_id: Annotated[int, typer.Argument(help="ID of the task to delete")]) -> None:
